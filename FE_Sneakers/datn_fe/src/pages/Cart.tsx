...
const CartPage: React.FC = () => {
    ...
    const fetchCart = async () => {
        ...
        try {
            ...
            if (error.response?.status === 401) {
                toast.error(t("please_login_to_view_cart"), {
                    autoClose: 2000,
                });
                navigate("/login");
            } else {
                setError(
                    error.response?.data?.message || t("error_fetching_cart")
                );
            }
        } finally {
            setLoading(false);
    ...
    const handleUpdateQuantity = async (
        ...
        try {
            ...
            const totalItems = updatedCart.data.items.reduce(
                (sum: number, item: any) => sum + item.quantity,
                0
            );
            updateCartCount(totalItems);
            ...
            toast.error(
                error.response?.data?.message || t("error_updating_cart"),
                { autoClose: 2000 }
            );
        }
    };
        ...
        try {
            if (removeAction === "update") {
                ...
                if (response.data.quantity === undefined) {
                    ...
                    setSelectedItems(
                        selectedItems.filter((id) => id !== itemToRemove.id)
                    );
                }
            } else if (removeAction === "delete") {
                const response = await axios.delete(
                    `http://localhost:8000/api/carts/remove/${itemToRemove.id}`,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem(
                                "token"
            ...
            toast.error(
                error.response?.data?.message || t("error_removing_item"),
                { autoClose: 2000 }
            );
        } finally {
    ...
    return (
        <>
            <Helmet>
                <title>Giỏ hàng - Pole Sneakers</title>
                <meta name="description" content="Giỏ hàng - Pole Sneakers" />
                <meta
                    property="og:description"
                    content="Giỏ hàng - Pole Sneakers"
                />
