...
const ProductCate = () => {
  ...
  const { id } = useParams<{ id: string }>() // Lấy id từ URL (nike hoặc adidas)
  ...
  // Ánh xạ id từ URL với id số trong database
  const categoryIdMap: { [key: string]: number } = {
    nike: 1, // Giả sử id của danh mục Nike là 1
    adidas: 2 // Giả sử id của danh mục Adidas là 2
  }

  useEffect(() => {
    const fetchProducts = async () => {
      if (!id) {
        toast.error('Không tìm thấy ID danh mục.', { autoClose: 2000 })
        setIsLoading(false)
        return
      }

      const categoryId = categoryIdMap[id.toLowerCase()]
      if (!categoryId) {
        toast.error('Danh mục không hợp lệ.', { autoClose: 2000 })
        setIsLoading(false)
        return
      }
      ...
      try {
        ...
        toast.error('Không thể tải sản phẩm. Vui lòng thử lại sau.', { autoClose: 2000 })
        setProducts([])
      } finally {
        setIsLoading(false)
      }
  ...
  const SkeletonLoading = () => (
    <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-8'>
      {Array(itemsPerPage)
        .fill(0)
        .map((_, index) => (
          <div key={index} className='bg-white shadow-lg rounded-lg p-4 text-center relative h-[18rem] animate-pulse'>
        ...
        <>
          <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-8'>
            {currentProducts.length > 0 ? (
              currentProducts.map((product) => (
                ...
                >
                  <div className='absolute top-0 left-0 bg-red-600 text-white text-xs px-2 py-1 rounded-tr-md z-10'>
                    <p className='text-sm text-white-500'>-20%</p>
                  </div>
                  ...
                  <h3 className='text-sm mb-2 whitespace-nowrap overflow-hidden text-ellipsis'>{product.name}</h3>
              ...
              <p className='col-span-full text-center text-gray-500'>Không có sản phẩm nào để hiển thị.</p>
            )}
          </div>
