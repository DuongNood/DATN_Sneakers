...
const ProfilePage = () => {
    ...
    const handleInputChange = (e, field) => {
        const value =
            field === "image_user" ? e.target.files[0] : e.target.value;
        console.log(
            `Input changed - ${field}:`,
        ...
        if (field === "image_user") {
            toast.warn("Vui lòng nạp VIP để thay đổi ảnh.", {
                autoClose: 3000,
            });
            return;
        }
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.errors) {
                    const backendErrors = Object.values(errorData.errors)
                        .flat()
                        .join(", ");
                ...
                throw new Error(
                    errorData.message ||
                        `${t("update_error")}: ${response.status}`
                );
            }
            ...
            if (error instanceof yup.ValidationError) {
                ...
                error.inner.forEach(
                    (err) => (newErrors[err.path] = t(err.message))
                );
                setErrors(newErrors);
