    ...
    private function createVNPayPaymentUrl($order)
{
    ...
    // ✅ GHI LOG ĐỂ KIỂM TRA LẠI
    \Log::info('=== VNPAY DEBUG (AFTER CONFIG) ===');
    \Log::info('HashData: ' . $hashData);
    \Log::info('SecureHash (Generated): ' . $vnpSecureHash);
    \Log::info('vnp_HashSecret (Used): ' . $vnp_HashSecret);
    \Log::info('vnp_ReturnUrl (Used): ' . $vnp_Returnurl);
    \Log::info('vnp_Url (Used): ' . $vnp_Url);
